...
import config from "./ojs-config.json" with {type: "json"};
...
class Grouper extends Splitter {
	...
	async classesFromFilePath(filePaths) {
		...
		for (let filePath of filePaths) {
			...
			for (let [key, { code, name, extends: parent, signature }] of map) {
				if (
					/OpenScript\.Mediator/.test(parent) &&
					!/.*Mediator$/.test(name)
				) {
	...
	async wrapClassesFromFilePath(filePaths) {
		try {
			...
			let finalContent = [];
			// get prefix contents
			for (const dir in config.content.prefixFiles) {
				for (let path of config.content.prefixFiles[dir]) {
					let content = fs.readFileSync(`${dir}/${path}`, "utf-8");
					finalContent.push(content);
				}
			}

			finalContent.push(finalCode);

			// get suffix contents
			for (const dir in config.content.suffixFiles) {
				for (let path of config.content.suffixFiles[dir]) {
					let content = fs.readFileSync(`${dir}/${path}`, "utf-8");
					finalContent.push(content);
				}
			}

			let final = finalContent.join("\n");
			this.consolidateToFile(final);
		} catch (error) {
	...
	async consolidateToFile(consolidatedCode) {
		const consolidatedFile = config.outputFile.name;
		fs.writeFileSync(consolidatedFile, consolidatedCode, "utf-8");

		// const [error, data] = await tryToCatch(minify, consolidatedFile, {
		// 	js: {
		// 		putout: {
		// 			quote: "'",
		// 			mangle: true,
        ...
        // fs.writeFileSync(config.outputFile.minified, data, "utf-8");
