...
export default class Router {
  ...
  on(path, action, name = null) {
    let _path = `${this.path}/${this.__prefix.join("/")}/${path}`.replace(
      /\/{2,}/g,
      "/"
    );
  ...
  listen() {
    ...
    container.resolve("broker").send("ojs:beforeRouteChange");
    ...
    container.resolve("broker").send("ojs:routeChanged");
  ...
  toName(routeName, params = {}) {
    ...
    for (let x of rn.route.match(/\{[\w\d-_]+\}/g) ?? []) {
      ...
      if (!v) {
        throw Error(`${rn.route} requires ${x} but it wasn't passed`);
      }
