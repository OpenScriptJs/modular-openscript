        ...
        h.div(
          ...
          h.div(
            ...
            ...sampleProducts.map((product) =>
              h.div({ class: "col-md-4 mb-3" }, h.ProductCard(product))
            )
          )
        ),
        h.div(
...
function initializeApp() {
  // Initialize mediator
  // With IoC, we should register it or just instantiate it if it registers itself
  const cartMediator = new CartMediator();

  // Mount the dashboard component to DOM
  // ojs() handles mounting to the root element defined in context or default
  ojs(Dashboard);

  // Broadcast system booted event
  app("broker").broadcast($e.system.booted);
}
